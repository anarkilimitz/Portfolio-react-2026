...
## 6. Оптимизация изображений и production-сборка

Настроил отдельный этап оптимизации изображений после Vite-сборки. Исходные PNG/JPG остаются в `src`, а оптимизированные изображения формируются непосредственно в `dist`.

- **Vite** — используется для основной production-сборки приложения и формирования итоговых asset-файлов.

- **WebP** — PNG/JPG после сборки конвертируются в WebP с качеством `80`.

- **Sharp** — изображения с большими разрешениями автоматически уменьшаются до максимального размера `1250×1250` без увеличения исходных изображений.

- **Оптимизация только при необходимости** — если WebP получается больше исходного изображения, конвертация не используется, а оригинальный файл сохраняется.

- **Удаление исходных файлов из production** — если WebP меньше оригинала, исходный PNG/JPG удаляется из `dist`.

- **Автоматическая замена ссылок** — после генерации WebP ссылки на изображения в собранных `JS`, `CSS` и `HTML` автоматически заменяются на соответствующие `.webp`.

- **Исходники не изменяются** — компоненты продолжают импортировать исходные PNG/JPG из `src`, поэтому production-оптимизация не требует изменения существующего React-кода.

- **Отдельный production-скрипт** — оптимизация вынесена в `scripts/generate-webp.mjs` и запускается автоматически после `vite build`.

**Результат оптимизации:**

| Показатель | До | После |
|---|---:|---:|
| Размер изображений | 22.92 MB | 1.49 MB |
| Экономия | — | 21.43 MB |
| Снижение размера | — | **93.5%** |
| Lighthouse Performance | 68 | **99** |
| FCP | 2.8 s | **0.6 s** |
| LCP | 2.9 s | **0.7 s** |
